// Phone parsing / E.164 / validation: E.164 conversion, per-country validation,
// and paste resolution. The catalog/lookups live in the ./phoneData,
// ./callingCodeDefaults, ./areaCodes and ./locale modules and the mask engine in
// ./phoneMask; all are re-exported from the public ./phone entry alongside this
// file. Holds no external value imports of its own (only types), so bundling it
// never drags a country-data-ts constant into a consumer that doesn't use one.

import type { CountryCode } from 'country-data-ts/countries';
import type { CountryPhoneConfig } from 'country-data-ts/phone-data';
import { resolveAreaCountry } from './areaCodes';
...
// Pure calling-code helpers (getCallingCodeDigits, trimTrunkPrefix) live at the
// end of the file so all exports stay grouped there; they're hoisted, so the
// parse functions above can call them freely.
